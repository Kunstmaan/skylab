        ...
        if (!$this->isUser($userName)) {
            if ($this->isDarwin()) {
                ...
                $this->processProvider->executeSudoCommand(
                    'dscl . create /Users/'.$userName.' NFSHomeDirectory '.$this->app["config"]["projects"]["path"]."/".$userName
                );
                ...
                $this->processProvider->executeSudoCommand(
                    'adduser --firstuid 1000 --lastuid 1999 --disabled-password --system --quiet --ingroup '.$groupName.' --home "'.$this->app["config"]["projects"]["path"]."/".$userName.'" --no-create-home --shell /bin/bash '.$userName
                );
            }
        }
    }
        ...
        if ($this->app["config"]["develmode"] || !$this->processProvider->commandExists("setfacl")) {
            ...
            if (!file_exists($this->fileSystemProvider->getProjectDirectory($project["name"]).'/.ssh/')) {
                $this->dialogProvider->logNotice(
                    $this->fileSystemProvider->getProjectDirectory($project["name"]).'/.ssh/'." does not exist, do not chmod"
                );
            } else {
            ...
            foreach ($permissions_sorted as $pd) {
                ...
                foreach ($pd->getAcl() as $acl) {
                    if (file_exists($path)) {
                        $this->processProvider->executeSudoCommand(
                            'setfacl '.$this->projectConfigProvider->searchReplacer($acl, $project).' '.$path,
                            true
                        );
                    } else {
